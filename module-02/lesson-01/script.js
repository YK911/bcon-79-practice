...
btn3.onclick = f3;
function f3() {
  const a = +input31.value;
  const b = +input32.value;
  if (a > b) {
    out3.textContent = a;
  } else {
    out3.textContent = b;
...
function f4() {
  const age = new Date().getFullYear() - +input4.value;
  if (age >= 18) {
    out4.textContent = 1;
  } else {
    out4.textContent = 0;
...
function f15() {
  const num1 = +select151.value;
  const num2 = +select152.value;
  const operation = select153.value;

  switch (operation) {
    case '&&':
      out15.textContent = num1 && num2;
      break;
